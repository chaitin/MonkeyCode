...
## 更新检查改为"切前台触发 + 30 分钟节流"(2026-07-25)

改前的节奏:壳启动后 5 秒查一次(会弹窗问是否更新,debug 跳过)、UI 挂载时查
一次 + **每 4 小时**静默复查、手动(托盘/设置页)。切前台不查,所以长驻不重启
的用户最长要等 4 小时才看得到新版。

- [x] `updateGate.ts`:30 分钟闸门。时钟从参数注入(测试要拨表);被挡掉的触发
      **不刷新计时**,否则每分钟切一次前台会把检查饿死;系统时间回拨(改时钟/
      NTP 纠偏)判为"隔了很久"放行,按差值算会是负数而闸死到时间追上为止
- [x] `App.tsx`:挂载、window focus、4 小时兜底三个触发点共用闸门。前台信号
      沿用同文件里云端刷新那处的 `window focus`(App.tsx:202),不另加
      `visibilitychange`——Tauri webview 对 visibilityState 的处理本就不一致,
      而密集触发反正由闸门吸收。桌宠是另一个 webview(pet.html),不受影响
- [x] 4 小时兜底保留:窗口一直开着没失去过焦点(挂着跑长任务)就等不到前台事件
- [x] 手动检查不过闸门(用户明确要查就得查)但 `record()` 记账,免得刚查完
      切个窗口回来又查一遍。设置视图与 App 同在 main 这一个 webview 里
      (`emit_to("main", "open-settings")`,不是独立窗口),模块级单例闸门因此
      是真:app 级的
- [x] 设置页导航"通用" → "通用与更新"

测试 6 例,反向验证过:把实现改成"被挡掉也记账"挂 2 例、去掉时间回拨保护挂 1 例。

- [x] 收掉壳启动 5 秒那条并行路径(原 `main.rs:862`):它与 UI 挂载那次在启动
      瞬间各打一遍更新端点,且它的弹窗与侧栏横幅重复。自动检查现在整条收在
      UI 侧,托盘的「检查更新」仍在。`MC_UPDATE_MANIFEST` 联调不受影响——它在
      `build_updater` 里覆盖清单地址,对托盘与 `update_check` 命令同样生效
- [x] 顺带收掉 `check_update`/`update_notice` 的 `manual` 参数:自动检查一走,
      它恒为 true,留着就是死参数(它存在的唯一理由就是让自动检查静默)
      **代价**:配置损坏走 error.html 时页面里没有 UI,不再有自动更新提示;
      托盘在那之前就建好了,手动检查仍可用
