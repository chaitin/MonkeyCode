...
test("skills are fully translated in every supported language", () => {
  const expectedPaths = getLeafPaths(enUS.pages.skills).sort()

  for (const [language, resource] of Object.entries(resources)) {
    assert.deepEqual(
      getLeafPaths(resource.pages.skills).sort(),
      expectedPaths,
      `${language} is missing skill translations`
    )
  }
})
