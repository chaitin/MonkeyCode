...
test("knowledge bases are fully translated in every supported language", () => {
  const expectedPaths = getLeafPaths(enUS.pages.knowledgeBases).sort()

  for (const [language, resource] of Object.entries(resources)) {
    assert.deepEqual(
      getLeafPaths(resource.pages.knowledgeBases).sort(),
      expectedPaths,
      `${language} is missing knowledge base translations`
    )
  }
})
