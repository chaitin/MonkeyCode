...
本期不实现专家草稿、发布版本、回滚和历史版本。专家 Prompt 及关联的 Connector Provider、Rule 和 Skill 修改即成为服务端最新配置，并影响后续新任务。
...
本期 Expert 固定为单 Agent，数据结构不预留成员表、角色、团队拓扑或成员级资源范围。未来实现专家团时重新设计，不要求当前 Expert 表无损升级为团队结构。
...
- 系统专家只能关联系统知识库；
- `expert_knowledge_bases` 包含 `expert_id`、`knowledge_base_id`、`sort_order` 和 `created_at`；
- 同一专家不能重复关联同一知识库；
