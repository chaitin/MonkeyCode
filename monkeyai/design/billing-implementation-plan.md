...
- **分组额度是用户默认额度模板**。用户各自拥有积分账户，分组不另设共享消费池。截图中根分组 15,000、管理员分组 50,000 可以同时成立。
- 用户显式额度优先；否则沿用户的计费归属分组向上查找最近显式额度，最终回落根分组。`null` 表示继承，`0` 表示该周期没有可消费额度，负值非法。
- 当前授权关系允许用户属于多个分组。推荐单独设置一个“计费归属分组”，授权关系仍可多选。未指定时管理员默认归属管理员分组，其他用户归属根分组；不得把多组额度相加，也不得按数据库返回顺序选一个。
- 分组页补齐创建、改名、移动、删除及成员归属维护的最小能力，复用根分组与管理员分组语义。禁止环和删除根分组；删除仍有子组或计费归属用户的分组时要求先迁移。
...
- 配置 JSON 中的 `root_credits` 迁入真实根分组额度；仅迁移能验证用户/分组存在的 UUID 覆盖。`engineering`、`member-01` 等模拟 ID 输出迁移报告，由管理员处理，不按名称猜归属。
...
| 场景 | 预期 |
| --- | --- |
| 根 15,000、管理员组 50,000、用户继承/覆盖/零额度 | 按确定的计费归属逐级解析，不相加、不随机取组 |
