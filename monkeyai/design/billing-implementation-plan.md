...
> 2026-09-18 分组额度统一：改为直接使用 `group_users`，多组取最高有效额度，不再提供个人周期额度；移除独立计费归属字段和接口。实现与本地交付检查已完成：计费/分组/身份模块数据库测试、应用层集成测试、旧归属迁移测试、前端 55 项测试、类型检查、变更文件 lint、生产构建及 SQL 生成一致性检查通过；未部署。历史实施记录中的单一计费归属方案已被替代。
...
本轮真实计费对象为系统模型调用、集中认证的 MCP 工具调用。用户自行配置的模型及其分享调用不收费。知识库执行服务目前尚未实现；截图里的知识库与“其他能力”示例不能作为真实账单保留。知识库后续复用计费服务接入，无需为本轮计费管理新建一套知识库业务。规则、技能与专家配置本身不单独收费。
...
- **分组额度是用户的周期额度模板**。用户各自拥有积分账户，分组不另设共享消费池。新系统默认启用直接计费，每周重置团队默认额度 10,000；自定义分组额度可以与团队默认并存。
- 分别沿用户直接所属分组向上查找最近显式额度，未配置的分支回落团队默认额度（计费设置 `root_credits`），最后取各分支有效额度的最高值；不提供个人周期额度。`null` 表示分组继承，`0` 表示该周期没有可消费额度，负值非法。
- 授权与额度继承共用 `group_users`，不再另设计费归属。未加入分组时继承团队默认，用户角色不决定归属；多组额度不相加。同额按直接所属组的创建时间、ID 稳定选择来源。
- 分组页支持创建、改名、移动、删除及成员归属维护，团队根节点仅用于展示，不入库，不预置管理员组。禁止环；删除仍有子组的分组时要求先迁移，成员额度则自动按剩余分组继承。
...
- 余额分为账面剩余、冻结和可用：`可用积分 = 账面剩余 - 冻结积分`。界面明确显示三个口径，避免将“周期额度”等同于“当前余额”。
...
- 修改刷新周期后，下次刷新取新规则的下一自然边界（上海时间每日零点、周一零点或月初零点）。切换前保留当前账户的起点、余额和冻结额，只缩短或延长其到期时间，不立即补发积分；切换时开启新规则的完整自然周期。再次修改仍保留原账户起点，防止重复发放；页面明确区分当前规则与待生效规则。
...
- 服务端依据数据库中的模型归属判定是否收费。`ownership_type=user` 的模型采用零价格快照，不冻结或扣减本地积分，不要求绑定百智云身份，也不调用钱包预扣费或确认扣费；保留零费用交易与 Token 用量统计，不强制计费输出上限。系统模型继续按下述规则计价。
...
- 后台「计费设置」配置服务 URL（base_url）、应用 ID、客户端证书、私钥和 CA，保存后无需重启；部署环境变量作为兼容回退。后台只展示配置状态、服务 URL 和证书到期时间，不回显凭据；取消“API Key 就能完成远程计费”的旧表单假设。
...
- 从 `/api/v1/settings` 的 `settings.billing` 下发的计费信息采用白名单，只含调用方需要的模式、价格说明和能力；不下发其他用户额度、全量覆盖表、钱包地址秘密或证书信息。
...
| 表或数据 | 调整内容 |
| --- | --- |
| `settings` | 保存全局计费策略和私有 `wallet` 连接配置，共用业务修订号；`schema_version` 继续表示结构版本，不能充当并发版本 |
| `billing_quotas` | 作为分组额度的唯一来源；真实 UUID、继承清除、修改人和审计，移除设置 JSON 双写；团队默认保存在计费策略中 |
| 用户分组与额度继承 | 统一读取 `group_users`，移除 `users.billing_group_id`；现有账户和流水的分组快照保留 |
...
| `wallet_billing_records`（新增） | 唯一 `biz_id`、交易关联、外部用户、服务 URL/应用快照、冻结与实扣 quota、请求状态、确认时间和重试信息 |
...
| `PATCH /api/admin/v1/billing/settings/{section}` | 只保存价格、周期、模式或钱包连接中的一个配置块；显式枚举 section，携带修订号，冲突返回 409 |
| `GET /api/admin/v1/billing/quotas` | 按组展开或搜索真实额度树、分组配置值、成员生效值、继承来源和直接所属分组 `group_ids` |
| `PUT /api/admin/v1/billing/quotas` | 原子保存本次分组额度变更集合，省略值表示不变，`null` 恢复继承；返回影响人数和生效时间 |
| `POST /api/admin/v1/billing/quotas/reset` | 按分组（含后代）和成员去重后立即重置当前额度；保留冻结积分，写重置流水与审计，并使用幂等键防止重复提交 |
...
- 左侧按实际成员关系显示分组和成员，顺序与成员管理一致；多组成员可在所属组下显示，但共用同一个独立账户。搜索与树形列表按同一继承规则预览分组额度草稿。
- 分组额度编辑保留“继承/自定义”；成员只显示当前剩余积分和继承到的周期额度。保存分组额度不隐式重置余额。
- 用户额度详情显示当期消耗、冻结、可用和下次刷新；当前周期临时补发或回收使用独立的“调整当期积分”，批量恢复到有效分组额度使用“立即重置”。
...
- 团队默认额度保存在配置 JSON 的 `root_credits` 中，版本 3 将旧真实根分组额度迁回该设置；仅迁移能验证分组存在的 UUID 配置。`engineering` 等模拟 ID 输出迁移报告，由管理员处理，不按名称猜归属。
...
| 场景 | 预期 |
| --- | --- |
| 团队 10,000、自定义分组 50,000、分组继承/零额度 | 按成员所属分组逐级解析，多组取最高有效额度，不接受个人周期额度 |
...
1. 分组提供成员周期额度模板，共用成员分组关系，多组取最高有效额度；不提供个人周期额度，也不引入分组共享余额池。
...
- 真实分组维护、成员授权关系与单一计费归属；团队默认、分组额度配置及继承。
...
- [x] 限制未完成交易期间的环境/应用切换；同一应用支持证书轮换。
- [x] 查询和审计脱敏，补充接口契约与运维说明。
- [x] 完成后端 PostgreSQL/RustFS 全量测试、计费竞态检查、`go vet` 和 sqlc 一致性检查。
- [x] 完成前端 45 项测试、lint、生产构建及 API 引用/迁移冲突检查。
- [x] 通过真实后台页面上传、保存、刷新恢复、错误提示和 390px 窄屏检查，表单无横向溢出，所有输入关联标签。

浏览器截图接口持续超时，本轮页面验证依据真实交互、语义树和布局尺寸，未完成像素级截图复核。

保存执行本地证书校验，不调用真实钱包；真实钱包联调仍需实际环境、应用 ID、证书及账户。


## 2026-09-09 单个百智云服务 URL

> 状态：实现与验收已完成，本轮同步最新 main 并提交 PR，进入代码审查阶段。

- [x] 拉取 main 并建立 `feat-monkeyai-wallet-url` 独立 worktree。
- [x] 环境选择替换为单个服务 URL 输入框；保存接口与响应统一使用 `base_url`。
- [x] 私有化服务的用户和钱包请求共享统一网关地址；公有云根地址自动映射对应服务。
- [x] 支持 HTTPS 根地址、内网 IP 和端口，校验路径、参数及用户信息并规范化地址。
- [x] 迁移旧 dev/prod 配置和交易快照，继续限制未完成交易期间的服务地址/应用切换，兼容部署环境变量。
- [x] 通过 PostgreSQL/RustFS 后端全量测试、双向 TLS 自定义网关验证、`go vet`、sqlc 一致性及 API/迁移检查。
- [x] 通过前端 43 项测试、lint 和生产构建。
- [x] 通过组件验证页检查单 URL 提交、非法地址拦截、标签关联和 390px 窄屏无横向溢出；保存由回调模拟，服务端保存由集成测试覆盖。

本次浏览器截图接口超时，未完成像素级截图复核。

真实私有化百智云的网关路由和账户权限仍需使用实际部署环境联调。

## 2026-09-09 用户模型免计费

> 状态：修复与本地验收完成，本轮提交 `fix-custom-model-billing` 到 `main` 的 PR，进入代码审查阶段；尚未发布。

- [x] 拉取 main 并建立独立 worktree。
- [x] 按数据库模型归属固化零价格，保留调用统计，避免用户模型进入本地积分扣减和钱包流程。
- [x] 覆盖本地积分耗尽、钱包未绑定/已绑定、使用他人模型和未配置计费输出限制；验证实际 HTTP 调用零费用结算。
- [x] 通过 PostgreSQL/RustFS 后端全量测试、`go vet`、sqlc 一致性及 API/迁移冲突检查。
