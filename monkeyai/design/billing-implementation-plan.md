...
> 2026-09-18 分组额度统一：改为直接使用 `group_users`，多组取最高有效额度，个人自定义额度优先；移除独立计费归属字段和接口。实现与本地交付检查已完成：计费/分组/身份模块数据库测试、应用层集成测试、旧归属迁移测试、前端 55 项测试、类型检查、变更文件 lint、生产构建及 SQL 生成一致性检查通过；未部署。历史实施记录中的单一计费归属方案已被替代。
...
- 用户显式额度优先；否则分别沿用户直接所属分组向上查找最近显式额度，未配置的分支回落团队默认额度（计费设置 `root_credits`），最后取各分支有效额度的最高值。`null` 表示继承，`0` 表示该周期没有可消费额度，负值非法。
...
| `GET /api/admin/v1/billing/quotas` | 按组展开或搜索真实额度树、覆盖值、生效值、继承来源和直接所属分组 `group_ids` |
...
- 左侧按实际成员关系显示分组和成员，顺序与成员管理一致；多组成员可在所属组下显示，但共用同一份个人额度和账户。搜索与树形列表按同一继承规则预览草稿。
...
| 场景 | 预期 |
| --- | --- |
| 团队 15,000、自定义分组 50,000、用户继承/覆盖/零额度 | 按成员所属分组逐级解析，多组取最高有效额度，个人覆盖优先 |
...
1. 分组提供个人额度模板，共用成员分组关系，多组取最高有效额度；不引入分组共享余额池。
