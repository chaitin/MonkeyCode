...
验证码使用安全随机六位数字，10 分钟有效，仅保存绑定邮箱和用途的摘要。数据库事务保证单次消费，最多允许 5 次错误尝试。按连接来源 IP（不信任转发头）和规范化邮箱限制发送：每邮箱每分钟 1 次、每小时 10 次，每 IP 每小时 30 次。SMTP 失败也占用发送额度，防止重试绕过限制。发送接口不通过成功提示区分账号是否存在；未开放注册的未知邮箱、停用或不符合用途的账号不发送邮件。验证码与限流记录保存在 PostgreSQL，支持多实例；每次发送清理过期记录。
