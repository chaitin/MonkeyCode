...
提供真实的授权对象查询。不预置分组，团队根节点仅作展示；资源授权按 `group_users` 的直接关系及未删除的上级分组解析，不按顶层位置或管理员角色自动授予。模型与其他资源使用同样的分组语义。新资源不默认引用旧系统根组，管理员须显式选择授权成员或真实分组。
...
- 集成测试验证规则保存、行级修订冲突、用户与父子分组授权、用户及分组强制规则、配置 ETag/304、技能上传与包重建、专家复制与依赖、委托下载和撤权、模板图标权限、真实 MCP HTTP 握手/发现、独立认证工具目录隔离，以及 OAuth state/PKCE 与回调防重放。
