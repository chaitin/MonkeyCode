...
| 模型管理 | 管理 CRUD、用户/分组授权、Agent 可用模型查询 | 复用接口和事务组织方式；专家选择真实模型 |
...
| 专家 | Prompt、资源关联、复制、启停 | 动态资源选项、默认模型、关系约束、依赖检查、资源清单 |
| 后端业务包 | `skill`、`rule`、`mcp`、`expert`、`resource` 主要只有 `doc.go` | 完整业务服务、数据访问、Admin/Agent DTO 和路由 |
| 下发 | `agentconfig` 已有 `/api/v1/config`、SHA-256 版本、ETag/304 | 增加四类资源，保证授权过滤、内容变更与删除均影响版本 |
...
- 个人技能和规则保留所有者及已有分享关系；管理后台展示来源、所有者和被引用情况，治理删除必须检查引用并记录操作者。
...
提供真实的授权对象查询。初始化根分组和管理员分组，并在可用性查询中明确“所有有效用户属于根分组、管理员分组按角色计算”，避免新用户缺少手工成员关系而收不到“所有用户”资源。其他已有分组按 `group_users` 和父子关系解析。现有模型查询同步使用这套分组语义，避免模型与其他资源的授权结果不同。本次不依赖组织架构编辑器才能完成授权。
...
保留 `experts.prompt`、名称、描述、启停和软删除，增加可空 `default_model_id`。使用 `expert_rules`、`expert_skills` 和 `expert_connector_providers` 维护关系。
...
- 默认模型仍需独立模型授权；不可用时返回可恢复问题，允许消费端选择其他已授权模型。
...
| 专家 | 新建、复制、编辑、启停、删除、授权；选择真实默认模型、Rule、Skill、Provider；设置必需依赖和工具过滤；预览有效清单与依赖问题 |
...
| `agentconfig` | 聚合脱敏资源，生成确定性的用户配置与 ETag |
...
| 接口 | 用途 |
| --- | --- |
| `GET /config` | 保留既有字段并增加规则、技能、专家、Connector 与工具目录描述 |
...
沿用 `/api/v1/config` 的拉取和 ETag，不增加配置通知服务。配置请求先检查有效用户身份，随后只聚合其可用资源。

| 顶层字段 | 内容 |
| --- | --- |
| `schema_version` | 下发契约结构版本，新增字段与既有客户端兼容 |
| `version`、`updated_at` | 全量内容摘要与所含资源最近更新时间；是否变化以 `version` 为准 |
| `settings`、`model_gateway`、`models` | 保留当前含义与字段 |
| `rules` | 直接可用规则的 ID、名称、正文、内容摘要、`required` |
| `skills` | 直接可用技能的 ID、名称、描述、标签、包大小、SHA-256、稳定下载路径 |
| `experts` | ID、名称、描述、清单摘要、清单路径、依赖问题及可用状态 |
| `connectors` | ID、Provider ID/identifier、名称、认证模式、当前用户认证状态、连接状态、目录路径/摘要 |
...
空集合统一为 `[]`。集合按稳定键排序，摘要只包含确定性业务字段，不包含当前请求时间、随机数据或短期下载签名。撤权、禁用、删除、分组成员变化会改变有效集合，因此即使剩余资源的 `updated_at` 没有增长也必须产生新版本。

配置生成使用同一数据库一致读视图；哈希覆盖实际响应内容及专家依赖摘要，不以最大更新时间充当唯一变更判断。批量查询关联资源，避免按每个专家分别查询。部分模块读取失败时整个配置返回失败，不能返回删空该模块的“成功配置”使客户端误删缓存。

### 7.2 专家清单与权限

清单包括 `expert_id`、版本、Prompt、默认模型、规则正文、技能元数据、Provider 依赖、候选 Connector 和结构化 `issues`。
...
专家清单的 `issues` 区分 `missing_connector`、`authorization_required`、`skill_disabled`、`model_unavailable` 等原因，并标记是否阻止使用。未授权的资源不泄露账号、凭证或私有目录内容。

### 7.3 客户端接入约定

客户端登录、重连和创建新工作单元时检查配置版本，轮询频率沿用现有配置同步约定。收到完整新快照后按用户和服务端隔离缓存，不能将缺失项继续当作已授权资源。
...
| E：专家闭环 | 默认模型、动态关联、复制与授权、依赖检查、委托清单 | 专家只有真实有效关系，缺失依赖有明确原因，委托访问不扩散 |
| F：集成与交付 | 完整配置 ETag、单版初始化与重新部署验证、原有样式对照、API 文档、端到端验收 | 四页样式延续现状，CRUD → 数据库 → 目标用户配置/技能下载全部连通 |

B、C、D 完成各自闭环后再集成 E，F 完成才标记本次功能交付。实现可以按业务能力拆分 PR，但单版初始化 SQL、`go.mod`、`internal/app`、公共协议和两份 OpenAPI 由集成任务统一维护。

必要验证覆盖：

- 用户 A 有权限、B 无权限；父组、根组、管理员组和撤权后的配置差异。
...
- [x] E：专家默认模型与资源关系、复制、授权、有效清单和委托下载。
...
- 集成测试验证规则保存、行级修订冲突、用户与父子分组授权、根组强制规则、配置 ETag/304、技能上传与包重建、专家复制与依赖、委托下载和撤权、模板图标权限、真实 MCP HTTP 握手/发现、独立认证工具目录隔离，以及 OAuth state/PKCE 与回调防重放。
