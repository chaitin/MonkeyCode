...
系统不初始化任何分组。团队根节点仅用于展示，不写入 `groups`，名称读取 `branding.workspace_name`；所有 `parent_id` 为空的记录都是团队根节点的直属子分组。管理员角色不产生分组或成员关系，所有真实分组的直接成员均通过 `group_users` 维护。团队根节点可新建子分组，其名称在团队设置中修改。
...
用于组织用户、承载授权范围并形成额度继承树。不预置分组；管理端可在虚拟团队根节点或已有分组下创建子分组。

| 字段 | PostgreSQL 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `id` | `uuid` | 是 | 自动生成 | 分组 ID，均由创建操作生成。 |
| `parent_id` | `uuid` | 否 | `NULL` | 父分组 ID；为空时表示直属虚拟团队根节点。 |
| `name` | `text` | 是 | 无 | 分组显示名称。 |
| `created_by_user_id` | `uuid` | 否 | `NULL` | 创建者用户 ID。 |
| `created_at` | `timestamptz` | 是 | 当前时间 | 创建时间。 |
| `updated_at` | `timestamptz` | 是 | 当前时间 | 名称或层级最近更新时间。 |
| `deleted_at` | `timestamptz` | 否 | `NULL` | 分组软删除时间；删除前须迁移子分组，成员后续额度按剩余分组自动继承。 |

### 5.4 `group_users`：分组用户关系

记录真实分组的直接成员；父分组成员与后代成员合并去重。虚拟团队根节点显示所有用户，不产生分组关系记录；用户角色不影响所属分组。额度继承共用此关系，不维护独立计费归属；无个人自定义额度时，分别解析各所属分组最近的显式额度并取最大值，无分组或整条分支未配置时继承团队默认。
...
没有额度记录表示继承最近上级分组，最终回落计费设置中的 `root_credits`。虚拟团队根节点不在本表创建额度记录。
