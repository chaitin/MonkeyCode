...
> 状态：已实施，数据库迁移为 `000012_mcp_connector_credentials`；发布操作见 [迁移与接入说明](connector-migration.md)。
>
> 日期：2026-09-11。
>
...
增量迁移与只读预检查已提供。部署时在实际数据库执行迁移，不重建既有连接和凭证。

### 12.1 数据保留

1. Connector 已有 URL 和 OAuth 配置快照，保留这些实例值；图标从原 Provider 复制对象引用，不覆盖实例凭据。共享同一图标对象时，清理必须检查所有引用。
2. 保留 Connector、Credential、Tool 的现有 ID，以及授权、分享、调用和计费历史。不能因去掉 Provider 重建所有连接。
3. 原 Provider 禁用时，其 Connector 的有效禁用状态必须迁移到 Connector，不能在删除模板后意外恢复可用。
4. 删除用户维度凭证唯一约束，添加名称、编辑版本和测试字段。原独立凭证使用“原有凭证”等可编辑名称，不猜测上游账号。
5. 原凭证 `method` 和 `status` 移除前检查数据：撤销记录保留撤销时间；已过期且无刷新能力或明确无效的旧认证不能被迁移成可用。认证内容按最终连接方式清理、派生状态。
6. 旧 Connector 的测试状态无法可靠归属每份用户凭证；独立凭证测试状态初始化为 `unknown`，不把某次用户测试复制成所有人的成功状态。有效工具目录继续保留。
7. 切换前使未完成的旧 OAuth 事务结束为失败或过期，避免旧“按用户更新”的回调在新结构下写入。用户重新发起授权。

### 12.2 专家依赖迁移

原 Provider 到 Connector 可能是一对多，不能自动把候选连接全部变成必需依赖，也不能随意选第一个。

- 只有一个符合专家归属及引用规则的有效 Connector 时，可以转换为直接关联，并保留 required 和工具过滤配置。
- 没有候选或有多个候选时，迁移预检查输出专家、原 Provider、候选连接和原因，要求在实际迁移前明确映射。
- 映射完整并验证后，才删除旧关联和 Provider。未解决映射时迁移停止，不静默删除依赖、不授予额外连接权限。

先执行 `backend/tools/connectorcheck.sql` 输出候选报告；有歧义时通过迁移连接参数 `monkeyai.connector_mappings` 指定映射。
...
- 后端、管理后台、Agent 消费契约和两份 OpenAPI 按同一版本发布。实际迁移窗口暂停旧写入，完成迁移后再启动新版本。
- 新版本出现多凭证数据后，不能直接恢复旧唯一约束。回滚需另行解决多凭证数据归并，或恢复完整备份；不提供会静默删除凭证的 down 迁移。

## 13. 实施拆分与验收

| 工作单元 | 主要范围 | 交付结果 |
| --- | --- | --- |
| 数据与 MCP 业务 | 增量迁移、`internal/mcp` SQL 与服务、sqlc 配置和生成结果 | 直接连接配置、凭证集合、OAuth/Header 多凭证、图标与网关 |
...
| 文档与迁移交付 | API 说明、资源设计、迁移预检查报告 | 文档一致，明确数据转换与版本切换步骤 |

实施时遵守现有业务目录和集成点约束，不手工修改 sqlc 生成文件；schema 从迁移重新生成。迁移序号按实施分支的最新 main 分配，不在本文预占。
...
实现与本地验收已完成，结果见 [实施计划](connector-implementation-plan.md)。生产数据预检查、迁移与部署尚未执行，发布时按 [迁移与接入说明](connector-migration.md) 完成；Desktop/OhMyAgent 外部加载器按已交付的 API 契约接入。
