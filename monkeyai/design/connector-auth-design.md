...
sequenceDiagram
    ...
    O-->>M: 返回 Token 和有效期
    M->>D: 再次校验版本及权限，写入指定凭证
    M->>O: 使用凭证初始化 MCP 并发现工具
    M->>D: 保存工具及测试状态，标记授权成功
    M-->>B: 授权结果页面
    A->>M: 查询事务结果
...
Token 交换完成后，在短数据库事务中重新检查上述权限及版本。新增操作插入新凭证；重新授权只更新指定凭证。凭证写入、原目录失效和新凭证 ID 回填一并提交，事务保持 `processing`。提交后由服务端自动测试连接并发现工具，完成此次测试尝试后再标记授权事务成功。
...
事务成功响应包含 `credential_id`。服务端已对这份凭证自动执行连接测试和工具发现，Agent 可直接读取工具目录；发现失败仍返回授权成功，并在凭证测试状态中记录错误，保留手动测试入口。重新授权也会自动重新发现，旧目录在写入新凭证时失效。自动测试使用独立的一分钟超时，不因浏览器关闭而取消。
...
- Access Token 即将到期：后台启动时及每 15 秒扫描具有 Refresh Token、当前配置有效且未撤销的凭证，提前 30 秒刷新；停用或删除的连接及用户不参与。每轮最多并发刷新 4 份凭证，单次超时 30 秒。调用或测试前仍对具体凭证加行锁并重新检查有效期，后台与请求并发时复用已刷新的令牌。
...
新增或替换 Header 提交后，服务端自动执行连接测试并更新工具目录，返回包含最新测试状态的凭证元数据；新增返回 `201`。测试失败仍保留已保存的凭证，由界面展示失败状态并提供重试入口。仅改名不触发测试。
...
5. 个人 Connector 和独立认证 Connector 新发现的工具自动启用，用户认证完成后即可从凭证工具接口、Agent 目录和 MCP `tools/list` 获取。系统 Connector 的集中认证及免认证新工具默认禁用，由管理员启用。重复发现保留系统工具已有启停状态。管理员按具体凭证上下文管理系统工具，不因此获得凭证秘密或替用户调用权限。
