...
> 状态：阶段一至四代码已在 `feat-image-proxy` 隔离 worktree 中集成（模型、迁移、原生代理、任务、按图计费、四条适配路径、管理配置及动态测试表单）。自动化单元检查已运行；缺少 PostgreSQL 测试库和真实上游凭证，数据库往返、真实供应商端到端与部署仍需验收。
>
> 日期：2026-09-21
>
...
3. 供应商代码按具体上游模型提供能力和参数映射，管理员只选择开放的画质、比例及积分；客户端在有效约束内组合参数。
...
同一供应商的不同模型可能支持不同的画质、比例、图片数量、参考图和格式。适配器代码按上游模型定义原生能力、合法组合和参数映射；管理员仅从中选择向客户端开放的画质和比例，并配置积分。模型目录是两者的交集，不允许管理员自行填写像素映射或声明适配器不支持的选项。
...
用户积分由 MonkeyAI 的模型定价配置决定；供应商真实费用只作为成本和对账信息记录。首版支持固定每张积分，并预留画质档位、比例和操作倍率。
...
`quality` 对应截图中的 1K/2K 等画质档位（并非跨厂商一致的像素数或主观清晰度等级），`aspect_ratio` 对应 1:1、4:3、3:4、16:9、9:16、3:2、2:3、21:9 等比例。格式、负面提示词等仍可按第 5.3 节能力配置扩展。参考图以本地上传后返回的 `file_id` 引用，不直接把临时 URL 或 Base64 塞进异步任务 JSON。
...
- 1K/2K 只表示该模型的画质档位，不承诺跨供应商统一宽高；具体像素由 Provider 按模型和比例转换并以实际输出为准。
...
画质和比例的支持范围及组合约束由供应商适配器按上游模型提供，管理员仅从支持范围中选择对外开放的画质、比例和默认选项。目录中的 `allowed_aspect_ratios` 由适配器能力与已开放选项求交得到，不能由管理员随意填写。Service 在模型保存及调用前校验开放选项属于适配器支持的组合；适配器升级后若不再支持某个已配置组合，拒绝相关调用并提示管理员调整，而不是静默换尺寸。返回结果记录上游实际宽高，不把 `2K` 推断为固定像素数。
...
type ImagePricing struct {
    ...
    QualityMultipliers    []NamedMultiplier `json:"quality_multipliers,omitempty"`
    AspectMultipliers     []NamedMultiplier `json:"aspect_ratio_multipliers,omitempty"`
    OperationMultipliers  []NamedMultiplier `json:"operation_multipliers,omitempty"`
}
...
积分和倍率在 API、JSON 和数据库快照中使用十进制字符串，计算使用现有定点金额能力，不经过 `float64` 或 JavaScript `Number`。
...
- 按客户端已选的画质档位和比例计算可预知价格，不依赖未知的实际输出像素数；
- 未配置的倍率视为 1；
- 配置必须无重名、正数，倍率键必须属于模型已公布的画质、比例或操作选项；
...
1. 根据请求图片数、画质、比例和操作计算最大可收费金额并冻结；
2. 冻结成功后才能提交上游；
3. 成功完成时按实际成功图片数结算；
...
```json
{
  ...
  "image_pricing": {
    "base_credits_per_image": "10",
    "quality_multipliers": [{"name": "2K", "multiplier": "2"}]
  }
}
```
...
```json
{
  ...
  "outputs": [
    {
      "url": "/v1/images/outputs/<output-id>",
      "mime_type": "image/png",
      "width": 2048,
      "height": 2048
    }
...
以上响应仅为示例：该模型把 2K、1:1 映射为 2048×2048，2K 每张 20 积分，冻结 60，最终只归档 1 张并扣除 20，释放剩余 40；其他模型的 2K 可能对应不同实际尺寸。首版不提供公开取消端点；客户端停止查询或断连不取消任务，后台仍需完成归档与计费。
...
记录 image_outputs
    ↓
返回需同一调用密钥鉴权的图片下载相对路径
```

Object Key 建议：

```text
image-outputs/<job-id>/<ordinal>-<sha256>.<ext>
```

采用不可变对象语义，不覆盖已有输出。首期返回 `/v1/images/outputs/{id}` 相对路径，下载需调用密钥并核对任务所有者，不签发预签名 URL；结果默认保留 30 天，到期清理图片对象，任务和计费记录继续保留，查询接口返回 `expired` 状态。
...
| xAI Grok 生图 | `xai` | 首期支持 `grok-imagine-image-2.0` 生成，按 `resolution` 映射 1K/2K；编辑接口无法保证统一画质/比例语义，暂不开放 Grok 编辑。 |
...
- 从适配器支持的档位中选择对外开放的画质及默认画质；
- 从适配器支持的比例中选择对外开放的比例及默认比例；
- 每张基础积分及可选画质、比例、操作倍率。

管理员不填写宽高、1K/2K 像素对应表、图片数量、操作能力、输出格式、参考图数量、Seed、Mask、Steps、Guidance 等参数。后台可只读展示适配器提供的其他能力，服务端在保存时校验画质、比例及其组合均受支持；非支持模型不能保存。区域、版本等接入必需项按适配器固定规则或专用基础接入配置处理，不暴露为管理员需维护的画质/比例映射。

后台提供显式“测试生成”操作，可在模型已开放的画质、比例、张数和生成/编辑能力内选择参数、上传参考图，并用调用密钥创建任务、轮询状态及鉴权下载图片；不在浏览器持久化密钥。保存模型不自动调用供应商。测试生成会产生上游费用，界面提前说明。
...
- 管理后台仅选择适配器提供的画质、比例及积分，映射和其他能力由适配器代码维护；
- 客户端可读取合成后的生图能力和公开价格。
...
- 客户端能读取模型级画质和比例选项并自由组合合法参数，不依赖输入像素宽高；
- 保存模型和调用前都校验所选画质/比例组合受适配器支持，不静默改成邻近尺寸；
- 适配器版本改变能力时，已保存但不再支持的组合被拒绝，并更新模型目录版本；
- 同步和异步供应商使用同一任务模型；
- 同一幂等键不会重复生成或重复扣费；
- 部分成功按实际成功图片数收费；
- 服务重启后能恢复有供应商任务 ID 的异步任务查询；同步调用提交后状态未知时保持 `unknown` 和待对账冻结，不盲目重新生成；
...
已确认：原生接口始终异步；结果图片默认保留 30 天；明确失败或内容审核拒绝全额退还积分，结果未知进入对账；编辑能力按具体模型开放。

首期任务表只保存 Prompt 对应请求哈希，不持久化明文 Prompt；同步上游调用的结果未知时保留冻结等待人工核查，账单管理员需要填写证据和已归档张数。若未来接入异步上游，保留任务 ID 并按 `ImageTaskQuerier` 恢复。Google/AWS 等后续云供应商使用工作负载身份还是统一 Secret/Credential 管理另行决定。
