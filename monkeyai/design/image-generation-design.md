...
积分和倍率在 API、JSON 和数据库快照中使用十进制字符串，计算使用现有定点金额能力，不经过 `float64` 或 JavaScript `Number`。倍率必须大于 0，允许最多 6 位小数；未配置的画质倍率按 1 计算。
...
- `quality_multipliers` 只允许为模型已开放的 1K、2K、4K 档位配置倍率，未配置时按 1 计算；
- 倍率为正十进制数，可以小于 1，例如 `0.5`；倍率名称不能重复；
...
| xAI Grok 生图 | `xai` | 首期支持 `grok-imagine-image-2.0` 生成，按 `resolution` 映射 1K/2K/4K；编辑接口无法保证统一画质/比例语义，暂不开放 Grok 编辑。 |
