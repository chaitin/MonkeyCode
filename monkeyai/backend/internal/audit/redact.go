...
func sanitize(value any, depth int) any {
	...
	case map[string]any:
		...
		for key, item := range v {
			...
			case "id", "name", "display_name", "model_id", "user_id", "user_ids", "group_id", "group_ids", "all_users", "owner_user_id", "resource_id", "resource_type", "subject_type", "subject_id", "type", "role", "status", "enabled", "protocol", "ownership_type", "access_level", "usage_requirement", "schema_version", "revision", "amount", "reason", "mode", "charging_mode", "quota_refresh_cycle", "root_credits", "input_credits_per_million_tokens", "cached_input_credits_per_million_tokens", "output_credits_per_million_tokens", "credit_multiplier", "context_window_tokens", "max_output_tokens", "supports_vision", "supports_reasoning", "tag_ids", "rule_ids", "skill_ids", "expert_id", "provider_id", "connector_id", "external_user_id", "method", "route", "body_omitted", "environment", "app_id", "configured", "credentials_configured", "certificate_expires_at", "source":
				out[key] = sanitize(item, depth+1)
			default:
				out[key] = "[REDACTED]"
			}
