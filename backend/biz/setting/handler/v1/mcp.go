...
func NewMCPHandler(i *do.Injector) (*MCPHandler, error) {
	...
	v1.GET("/tools", web.BindHandler(h.ListTools))
...
// ListTools 获取当前用户可见的 MCP Tool 列表
//
...
//	@Produce		json
//	@Security		MonkeyCodeAIAuth
//	@Success		200	{object}	web.Resp{data=domain.ListUserMCPToolsResp}	"成功"
//	@Failure		401	{object}	web.Resp									"未授权"
//	@Failure		500	{object}	web.Resp									"服务器内部错误"
//	@Router			/api/v1/users/mcp/tools [get]
func (h *MCPHandler) ListTools(c *web.Context, _ struct{}) error {
	user := middleware.GetUser(c)
	resp, err := h.usecase.ListTools(c.Request().Context(), user.ID)
	if err != nil {
		return errcode.ErrDatabaseQuery.Wrap(err)
	}
	return c.Success(resp)
