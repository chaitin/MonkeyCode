...
1. 先查**有没有更新的原生版本** —— `GET /app-version?platform=android|ios`，读 `ota-server/native-release.json`（你每次发新 APK/IPA 时手动更新里面的 `version` 和下载 `url`）。若 `version` > 已装的 `Constants.nativeAppVersion` → 提示"发现新版本 vX，去更新" → 打开下载/商店链接（**OTA 推不动原生，必须装新包**）。
